...
#Update master floor list to include newly crossed chambers
rFloorStatusFile = open(r'\\amat.com\Folders\Austin\Global-Ops\AMO\CPI_TestWorkCntr\TECH FOLDERS\ Irvin Carrillo\MDPFloor.txt', 'r')
temp = rFloorStatusFile.readlines()
rFloorStatusFile.close()
wFloorStatusFile = open(r'\\amat.com\Folders\Austin\Global-Ops\AMO\CPI_TestWorkCntr\TECH FOLDERS\ Irvin Carrillo\MDPFloor.txt', 'w')

for p in mdpTest:
    #Only add newly found chambers from test queue
    if p.occupied:
        temp[mdpTest.index(p)] = p.info()
        
for l in temp:
    wFloorStatusFile.write(l)
    
wFloorStatusFile.close()
